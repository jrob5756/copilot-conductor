...
**Acceptance Criteria**:
- [x] Tool-based approach successfully extracts structured output from tool_use blocks
- [x] Fallback JSON extraction works when Claude returns text instead of tool_use
- [x] Parse recovery sends follow-up message when JSON is invalid (fallback case only)
- [x] Output schema is converted to tool definition with proper input_schema
- [x] Tests verify both tool-based and fallback approaches
- [x] Error messages provide helpful debugging context
- [x] Works with complex nested schemas (objects, arrays, combinations)
