...
Copilot Conductor allows users to define agent workflows in YAML configuration files and execute them from the command line. Agents can be chained together, pass context between each other, use tools, and loop until conditions are met.
...
- **Easy distribution**: Install via `uvx copilot-conductor` or `pipx`
...
4. **Easy distribution**: `uvx copilot-conductor` provides a clean install experience without requiring a compiled binary.
...
│   └── copilot_conductor/
│       ├── __init__.py
│       ├── __main__.py          # Entry point (python -m copilot_conductor)
│       ├── cli/
│       │   ├── __init__.py
...
```python
# src/copilot_conductor/config/schema.py
...
```python
# src/copilot_conductor/providers/base.py
...
# src/copilot_conductor/providers/copilot.py
...
# src/copilot_conductor/providers/factory.py
...
```python
# src/copilot_conductor/engine/workflow.py
...
```python
# src/copilot_conductor/executor/template.py
...
[project.scripts]
conductor = "copilot_conductor.cli.app:app"
