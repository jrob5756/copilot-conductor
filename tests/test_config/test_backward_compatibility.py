...
This module ensures that adding new Claude-specific fields to the schema
does not break existing Copilot workflows.

Tests verify:
1. All existing example YAML files load successfully
2. New Claude fields default to None when omitted
...
class TestBackwardCompatibility:
    ...
    def test_load_existing_copilot_workflows(self, example_file: Path):
        ...
        This verifies that adding new Claude-specific fields to the schema
    ...
    def test_copilot_workflow_with_new_schema_no_validation_errors(self):
        ...
        successfully with the new schema that includes Claude-specific fields.
    ...
    def test_serialization_round_trip_preserves_behavior(self):
        ...
        # Verify serialized dict does not contain new Claude fields
        runtime_dict = serialized.get("workflow", {}).get("runtime", {})
        
        # These fields should not appear in a Copilot workflow
        ...
        assert "top_p" not in runtime_dict or runtime_dict["top_p"] is None
        assert "top_k" not in runtime_dict or runtime_dict["top_k"] is None
        assert (
            "stop_sequences" not in runtime_dict or runtime_dict["stop_sequences"] is None
        )
        assert "metadata" not in runtime_dict or runtime_dict["metadata"] is None
    
    def test_claude_fields_default_to_none(self):
        """Test that new Claude-specific fields default to None when omitted.
        
        This verifies that workflows without Claude-specific configuration
        have those fields set to None by default.
        
        Task: EPIC-010-T4
        """
        # Create a minimal configuration without Claude fields
        config_dict = {
            "workflow": {
                "name": "test-defaults",
                "description": "Test workflow",
        ...
        # Verify Claude-specific fields default to None
    ...
    def test_exclude_none_prevents_claude_fields_in_copilot_configs(self):
        ...
        the new Claude-specific fields (which are None) don't appear in the output.
        ...
        # Verify new Claude fields are not present in the serialized output
    ...
    def test_all_examples_load_successfully(self, example_file: Path):
        ...
        # If it's a Claude workflow, verify Claude-specific fields can be set
        if config.workflow.runtime.provider == "claude":
            # Claude workflows can have these fields set or None
            # Just verify they're accessible (no assertion on values)
            _ = config.workflow.runtime.temperature
            _ = config.workflow.runtime.max_tokens
            _ = config.workflow.runtime.top_p
            _ = config.workflow.runtime.top_k
            _ = config.workflow.runtime.stop_sequences
            _ = config.workflow.runtime.metadata
    ...
    async def test_copilot_workflow_execution_backward_compatible(self):
        ...
        # Verify Claude fields are None (backward compatibility)
        runtime = config.workflow.runtime
        ...
        # Verify serialization doesn't include None Claude fields
        serialized = config.model_dump(mode="json", exclude_none=True)
    ...
    def test_claude_workflow_schema_validation(self):
        """Verify Claude workflow schema with new fields validates correctly.
        
        This test verifies that the schema correctly accepts Claude-specific fields
        and validates them appropriately. Integration testing with actual provider
        execution is covered in test_integration/test_schema_provider_integration.py.
        
        Task: EPIC-010-T5 (schema validation for Claude fields)
        """
        # Create a minimal Claude workflow with all new fields
        ...
        assert config.workflow.runtime.stop_sequences == ["STOP"]
        assert config.workflow.runtime.metadata == {"user": "test"}
        
        # Verify serialization includes all Claude fields
        serialized = config.model_dump(mode="json", exclude_none=True)
        runtime_dict = serialized["workflow"]["runtime"]
        ...
        assert runtime_dict["top_k"] == 40
        assert runtime_dict["stop_sequences"] == ["STOP"]
        assert runtime_dict["metadata"] == {"user": "test"}
    ...
    def test_missing_provider_defaults_to_copilot(self):
        ...
        # Verify no Claude fields are set
